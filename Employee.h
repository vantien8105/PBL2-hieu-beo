...
public:
    ...
    void addEmployee()
    {
        ...
        while (getline(infile, line))
        {
            ...
            string maNhanSu, hoTen, gioiTinh, ngaySinh, soDienThoai, diaChi, chuyenMon, congViec, maHopDong;
            double heSoLuong;
            string boPhan;

            
            iss >> maNhanSu >> hoTen >> gioiTinh >> ngaySinh >> soDienThoai >> diaChi >> chuyenMon >> congViec >> maHopDong >> heSoLuong >> boPhan;

            
            if (maNhanSu == nhanVienMoi->getMaNV() || maHopDong == nhanVienMoi->getMaHD())
            {
                isDuplicate = true;
                break;
            }
        ...
        if (isDuplicate)
        {
            cout << "Mã nhân sự hoặc mã hợp đồng đã tồn tại. Không thể thêm nhân viên.\n";
            delete nhanVienMoi;
            return;
        }
    ...
    void deleteNode(const string &maNhanVien, QuanLy *manager)
    {
        ...
        while (current)
        {
            ...
            if (nv && nv->getMaNV() == maNhanVien)
            {
                ...
                {
                    cout << "Phòng ban " << manager->getBoPhanLamViec()
                         << " không có nhân viên với mã " << maNhanVien << endl;
                    return;
                }
            }
            prev = current;
        ...
        ofstream outfile("employee_info.txt", ios::trunc);
